...
**Tech Stack:** React 19, **shadcn/ui** (copy-in, Radix + CVA + tailwind-merge), Tailwind v4 (`@tailwindcss/vite`), Be Vietnam Pro (`@fontsource`), CodeMirror 6 (`@uiw/react-codemirror`, `@codemirror/lang-json`, `@codemirror/lint`), WXT/Vite, TS strict, pnpm, Vitest.

> **Amendment (2026-07-08):** UI kit changed from **mvpui → shadcn/ui** (mvpui is an unpublished source-only monorepo, not consumable). **Task 1 is DONE** (commits `a1e3890`, `8a9ef6b`, `d125dd4`): shadcn toolchain + Tailwind v4 + Be Vietnam Pro (self-hosted) + the colorhunt palette + `cn`/Button installed and building. The `@` alias was **removed** (WXT hard-maps `@`→root and wins over overrides) — use **relative imports** everywhere. Read the mvpui/`data-theme` references in the tasks below through the shadcn mapping in Global Constraints.
...
- **UI kit: shadcn/ui (copy-in).** Components live under `src/console/ui/*.tsx` (add via `npx shadcn@latest add <name>` or copy from the shadcn registry), imported with **relative paths** (`./ui/button`, `../ui/dialog`) — **no `@/` alias** (removed). `cn` is at `src/console/lib/utils.ts`. Where the per-task JSX below imports from `@mvp-ui/ui`, use the shadcn equivalent: Button→Button, Input→Input, Label→Label, Modal→Dialog (`Dialog`/`DialogContent`/`DialogHeader`/`DialogTitle`), Select→Select (`Select`/`SelectTrigger`/`SelectValue`/`SelectContent`/`SelectItem`), Dropdown→DropdownMenu, Tag→Badge, Tooltip→Tooltip, Drawer→Sheet, Toggle→Switch, Tabs→Tabs. Add each component's Radix dependency as the shadcn registry specifies; adapt the example JSX to shadcn's real props.
...
- **Theme:** light/dark via the **`.dark` class** on `document.documentElement` (shadcn convention, NOT `data-theme`); default to the OS preference; expose a toggle. Palette (colorhunt) is already wired into the CSS token variables in `src/console/styles.css`: `#000000` black, `#233D4D` navy, `#FE7F2D` orange (`--primary`, white foreground), `#EAECF0` light gray.
